...
class Foto_lugar extends CI_Controller
{
    ...
    {
        $data['lugares'] = $this->General_model->get_data_dynamic('lugar_turisticoaux', "cod_lugar, name_lugar");
        $this->layout->view('foto_lugar/register', $data);
    }
    public function insert_foto_lugar()
    {
        $cod_lugar = html_purify($this->input->post('cod_lugar'));
        $response_1 = $this->General_model->get_data_dynamic('foto_atractivo', "count(cod_foto) as count, max(cod_foto) as max", false, array("cod_lugar" => $cod_lugar));
        ...
        if ($success) {
            $response = $this->General_model->insert_dynamic_array_('foto_atractivo', $array_tosend);
    ...
    {
        $foranea[0] = array(
            "tablefk" => "lugar_turisticoaux l",
            "comp" => "f.cod_lugar = l.cod_lugar"
        );
        $data['fotos_lugares'] = $this->General_model->get_data_dynamic('foto_atractivo f', "f.*, l.name_lugar", $foranea);
        $this->layout->view('foto_lugar/listar', $data);
    }
    public function editar($cod_foto = false)
    {
        if ($cod_foto || ($this->session->userdata('tipo_usu') == "SA" && $this->session->userdata('tipo_usu') == "AD")) {
            $data['lugares'] = $this->General_model->get_data_dynamic('lugar_turisticoaux', "cod_lugar, name_lugar");
            if ($data['lugares']) {
                $data['foto_lugar'] = $this->General_model->get_data_id_dynamic(
                    'foto_atractivo',
                    array('cod_foto' => $cod_foto)
                );
                $data['foto_lugar'] = isset($data['foto_lugar'][0]) ? $data['foto_lugar'][0] : false;
                // var_dump($data); die;
                $this->layout->view('foto_lugar/editar', $data);
            } else {
                redirect('foto_lugar/listar', "refresh");
            }
        } else {
            redirect('/', "refresh");
        }
    }
    public function delete_foto_lugar()
    {
        $cod_foto = html_purify($this->input->post('cod_foto'));
        $name_foto = html_purify($this->input->post('name_foto'));

        if ($cod_foto) {
            $response = $this->General_model->delete_dynamic('foto_atractivo', array("cod_foto" => $cod_foto));
    ...
    public function editar_foto_lugar()
    {
        $name_foto = $this->input->post('name_foto');
        $cod_lugar = $this->input->post('cod_lugar');
        $cod_foto = $this->input->post('cod_foto');
        $data = array("cod_lugar" => $cod_lugar);
        $where = array("cod_foto" => $cod_foto);
        $response = $this->General_model->edit_dynamic('foto_atractivo', $where, $data);
